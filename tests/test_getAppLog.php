...
//
// File: test_getAppLog.php - test web service that gets a sample log string from appLogs table
//
// Functions:
//    None
//
...
// What is the logId that needs to be retrieved? 0 retrieves the most recent value
$logId = 0;
...
// Value to be fetched from appLogs table
if (isset($_GET["l"])) {
    $logId = $_GET["l"];
}   //  End if (isset($_GET["l"]))

// STEP 1 - Positive use-case
// ********* Call Web Service with valid logId, verify you get back the saved value ********** //
$ch               = curl_init();

$elements          = array();
$elements["logId"] = $logId;
$elements["dump"]  = true;

curl_setopt($ch, CURLOPT_URL, $global_siteUrl . "services/getAppLog.php");
...
if ($errorCode === 0) {
    ...
    if ((isset($_GET["l"])) && ($logId > 0)) {
        echo("{\"logId\":" . $logId . ",\"log\":" . $checkResponse["log"] . "\"}");
    } else {
        echo("1: Pass. Found value: \"" . $checkResponse["log"] . "\" for logId: " . $logId);
    }   //  End if ((isset($_GET["l"])) && ($logId > 0))
} else {
